...
void cmdGenParticles(CmdBuffer cmdBuf, Buffer particleBuffer, Buffer predictedPosBuffer, GenParticleArgs args)
{
...
void cmdGenParticles(CmdBuffer cmdBuf, Buffer particleBuffer, Buffer predictedPosBuffer)
{
	...
	cmdGenParticles(cmdBuf, particleBuffer, predictedPosBuffer, args);
}

ComputeCmd getCmdUpdateParticles(Buffer particleBuffer, Buffer forceBuffer, UpdateParticleArgs args, Buffer predictedPos)
{
...
void cmdApplyTimeStep(CmdBuffer cmdBuf, Buffer particleBuffer, Buffer forceBuffer, float timeStep, Buffer predictedPos)
{
	VkRect2D_OP   targetImgSize = getScissorRect(viewDimensions);
	float         maxDim        = float(max(targetImgSize.extent.width, targetImgSize.extent.height));
	Rect2D<float> area{
	...
	getCmdUpdateParticles(particleBuffer, forceBuffer, updateArgs, predictedPos).exec(cmdBuf);
}

void cmdComputeInteractionData(CmdBuffer cmdBuf, Buffer posBuffer, const ParticleDescription &desc, SimulationResources &res)
{
	{
...

	{
		...
		getCmdComputePressureForce(posBuffer, res.densityBuffer, res.it, pressureCI, res.pressureForceBuffer).exec(cmdBuf);
	}
}
...
{
	cmdComputeInteractionData(cmdBuf, predictedPosBuffer, particle_type<glm::vec2>::get_description(gvar_particle_size.val.v_float * cParticle_size_scale), res);
	...
	cmdApplyTimeStep(cmdBuf, particleBuffer, res.pressureForceBuffer, timeStep, predictedPosBuffer);
}
