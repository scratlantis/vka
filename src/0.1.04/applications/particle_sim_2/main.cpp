...
GVar gvar_sliding_average_coef {"Sliding Average Coefficient", 0.0f, GVAR_FLOAT_RANGE, GUI_CAT_GENERAL, {0.0f, 1.f}};
...
int main()
{
	...
	while (doRestart)
	{
		...
		Image img_shaded_accumulation = createSwapchainAttachment(VK_FORMAT_R32G32B32A32_SFLOAT,
		                                             VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
		                                                 VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT,
		                                             VK_IMAGE_LAYOUT_GENERAL,
		                                             viewDimensions.width, viewDimensions.height);
		...
		while (!gState.io.shouldTerminate())
		{
			...
			if(reset || guiCatChanged(GUI_CAT_PARTICLE_GEN, settingsChanged))
			{
				...
				getCmdFill(img_shaded_accumulation, vec4(0.0)).exec(cmdBuf);
			...
			{
				MapImgArgs mArgs{};
				mArgs.slidingAverageCoef = gvar_sliding_average_coef.val.v_float;
				getCmdMapImg(img_shaded, img_shaded_accumulation, mArgs).exec(cmdBuf);
			}
			...
			// Composition
			{
				...
				else
				{
					...
					getCmdMapImg(img_shaded_accumulation, swapchainImg, mArgs).exec(cmdBuf);
				}
