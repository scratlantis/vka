#include "VrIOController.h"
#include <vka/core/stateless/vk_types/default_values.h>
#include <vka/globals.h>
namespace vka
{

void VrIOController::configure(IOControlerCI &controllerCI, Window *window)
{
...
void VrIOController::updateSwapchain()
{
...
bool VrIOController::swapchainRecreated()
{
	return swapchainWasRecreated;
}
Window *VrIOController::getWindow()
{
	return window;
}
void VrIOController::requestSwapchainRecreation()
{
	shouldRecreateSwapchain = true;
}

void VrIOController::readInputs()
{
...
void VrIOController::clearEvents()
{
	memset(&keyEvent, 0, VKA_KEY_COUNT * sizeof(bool));
	memset(&keyPressedEvent, 0, VKA_KEY_COUNT * sizeof(bool));
	memset(&keyReleasedEvent, 0, VKA_KEY_COUNT * sizeof(bool));
...
void VrIOController::terminateWindowManager()
{
	window->terminateWindowManager();
}

bool VrIOController::shouldTerminate()
{
	return window->shouldClose();
}

void VrIOController::buildShaderLib()
{
...
void VrIOController::requestTerminate()
{
	window->requestClose();
}
}        // namespace vka